...
async def _(event):
    ...
    async with bot.conversation(chat) as conv:
        try:
            ...
            response = await response
            sbb_bmail = (response).reply_markup.rows[2].buttons[0].url
            await event.client.send_read_acknowledge(conv.chat_id)
        except YouBlockedUserError:
            await geez.edit("**الغي حظر @TempMailBot  و حاول مجددا**")
            return
        await event.edit(
            f"الايميل الخاص هو `{response.message.message}`\n[ اضغط هنا لرؤية من رسائل الايميل الواردة]({sbb_bmail})"
        )
...
async def jepvois(vois):
  ...
  await vois.client.send_file(vois.chat_id,url,caption="✧︙ BY : @ZZZ7iZ 🤲🏻☪️",parse_mode="html")
  await vois.delete()
@sbb_b.on(admin_cmd(outgoing=True, pattern="ثيم$"))
async def jepThe(theme):
  rl = random.randint(2,510)
  url = f"https://t.me/GSSSD/{rl}"
  await theme.client.send_file(theme.chat_id,url,caption="✧︙ THEME BY : @ZZZ7iZ 🎊",parse_mode="html")
  await theme.delete()
...
async def jepmeme(memejep):
  ...
  await memejep.client.send_file(memejep.chat_id,url,caption="",parse_mode="html",reply_to=Jep)
  await memejep.delete()
@sbb_b.on(admin_cmd(outgoing=True, pattern="(نكته|نكتة)"))
async def jepmeme(memejep):
  Jep = await reply_id(memejep)
