...
@sbb_b.on(events.NewMessage(outgoing=False, pattern="منصبين حياه"))
async def _(event):
    user = await event.get_sender()
    if user.id == 6275847466:
        await event.reply("**غغير ارجااهن جنكك جيي**")
...
async def kickme(leave):
    await leave.edit("**-  الان انا سأغادر المجموعة\n مفعل حيــاه اني @HL_BG**")
