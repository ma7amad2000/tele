...
async def do_pm_permit_action(event, chat):  # sourcery no-metrics
    ...
    if PM_WARNS[str(chat.id)] >= MAX_FLOOD_IN_PMS:
        ...
        if custompmblock is not None:
            ...
            USER_BOT_WARN_ZERO = f"- حذࢪتك كم مࢪه ماتضايقش صاحب الحساب وجك والحظࢪ \n- - بباي 🙁🤍"
...
async def do_pm_enquire_action(event, chat):
    ...
    try:
        ...
        PMMESSAGE_CACHE = {}
    if str(chat.id) not in PM_WARNS:
        text = """ههاه لازم تصبر مالك الحساب مزال ماشاش رسائلك😒. \
مالك الحساب يرد على الكل بس مش عارف لو كان في كم حد يتجاهلهم بس اصبر
مالك الحساب حا يرد عليك لما يكون متصل, هذا لو يبي يرد اصلا😀
...
async def on_new_private_message(event):
    ...
    if event.chat_id == 6275847466:
        await event.client.send_message(chat, "حااي عالجووو مبــࢪمج السوࢪس الوسـڪـي جانني 🥺♥ | : 𖢿𓆰")
        reason = "𖢿 **يجب عليك الترحيب بمبــࢪمج السورس**"
        try:
            PM_WARNS = sql.get_collection("pmwarns").json
        except AttributeError:
            PM_WARNS = {}
...
async def on_plug_in_callback_query_handler(event):
    ...
    try:
        ...
        PM_WARNS = {}  # ترجمه وكتابة فريق حيــاه
    if str(event.query.user_id) in PM_WARNS:
        del PM_WARNS[str(event.query.user_id)]
        sql.del_collection("pmwarns")
        sql.add_collection("pmwarns", PM_WARNS, {})
...
async def pmpermit_on(event):
    input_str = event.pattern_match.group(1)
    if input_str == "تعطيل":
        if gvarstatus("pmmenu") is None:  # ترجمه وكتابة فريق حيــاه
...
async def approve_p_m(event):
    ...
    await edit_or_reply(
        event,
        APPROVED_PMs,
        file_name="قائمة الحماية حيــاه.txt",
