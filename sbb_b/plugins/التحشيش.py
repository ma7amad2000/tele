...
async def _(zed):
    await edit_or_reply (zed, "𓆰 [𝙎𝙊𝙐𝙍𝘾𝞝 𝙃𝘼𝙔𝘼 ༼༽](t.me/HL_BG) 🗑♻️𓆪\n**𓍹━─━─━─━─𝗛─━─━─━─━𓍻**\n\n **✵│رابـط الحـذف ↬** https://telegram.org/deactivate \n\n\n **✵│بـوت الحـذف  ↬** @LC6BOT ")

########################  SOURCE ZED ~ BY: ZelZal (@zzzzl1l)  ########################

@sbb_b.zed_cmd(pattern="رفع جلب(?: |$)(.*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
    if user.id in zel_dev:
        return await edit_or_reply(mention, f"**انت غبي هذا الوسڪي مطـوࢪ السورس🙂: 𖢿 |**")
    if user.id == 6119417529 or user.id == 6119417529 or user.id == 6195765774:
        return await edit_or_reply(mention, f"**انت معتوه هءا واحد من المطورين الاساسيين🙂: 𖢿 |**")
...
@sbb_b.zed_cmd(pattern="رفع مرتي(?: |$)(.*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
    if user.id in zel_dev:
        return await edit_or_reply(mention, f"**انت غبي هذا الوسڪي مطـوࢪ السورس🙂: 𖢿 |**")
    if user.id == 6195765774 or user.id == 6119417529 or user.id == 6119417529:
        return await edit_or_reply(mention, f"**انت معتوه هءا واحد من المطورين الاساسيين🙂: 𖢿 |**")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
    await edit_or_reply(
        mention,
...
@sbb_b.zed_cmd(pattern="رفع بكلبي(?: |$)(.*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@sbb_b.zed_cmd(pattern="رفع بقلبي(?: |$)(.*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@sbb_b.zed_cmd(pattern="رفع قلبي(?: |$)(.*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@sbb_b.zed_cmd(pattern="رفع جريذي(?: |$)(.*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
async def permalink(mention):
    ...
    if user.id in zel_dev:
        return await edit_or_reply(mention, f"**انت غبي هذا الوسڪي مطـوࢪ السورس🙂: 𖢿 |**")
    if user.id == 6119417529 or user.id == 6195765774 or user.id == 6195765774:
        return await edit_or_reply(mention, f"**انت معتوه هءا واحد من المطورين الاساسيين🙂: 𖢿 |**")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
    await edit_or_reply(
        mention,
...
ZelzalTHS_cmd = (
    "𓆩 [𓏺𝙎𝙊𝙐𝙍𝘾𝞝 𝙃𝘼𝙔𝘼 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - اوامـر التحشيش](t.me/HL_BG) 𓆪\n\n"
...
@sbb_b.zed_cmd(pattern="(نسبه الانوثة|نسبة الانوثه|نسبه الانوثه|نسبة الانوثة)(?: |$)(.*)")
async def zed(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
    if user.id in zel_dev:
        return await edit_or_reply(mention, f"**انت غبي هذا الوسڪي مطـوࢪ السورس🙂: 𖢿 |**")
    if user.id == 6195765774 or user.id == 6195765774 or user.id == 6119417529:
        return await edit_or_reply(mention, f"**انت معتوه هءا واحد من المطورين الاساسيين🙂: 𖢿 |**")
    zedth = user.first_name.replace("\u2060", "") if user.first_name else user.username
    zedt = random.choice(zid)
    await edit_or_reply(mention, f"**✾╎نسبـة الانوثه لـ**  [{zedth}](tg://user?id={user.id}) **هـي {zedt} 🤰**")
@sbb_b.zed_cmd(pattern="(نسبه الغباء|نسبة الغباء)(?: |$)(.*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@sbb_b.zed_cmd(pattern="(نسبه الانحراف|نسبة الانحراف)(?: |$)(.*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@sbb_b.zed_cmd(pattern="(نسبه المثليه|نسبة المثليه)(?: |$)(.*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@sbb_b.zed_cmd(pattern="(نسبه النجاح|نسبة النجاح)(?: |$)(.*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@sbb_b.zed_cmd(pattern="(نسبه الكراهية|نسبة الكراهيه|نسبه الكراهيه|نسبة الكراهية)(?: |$)(.*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
async def zed(mention):
    ...
    if user.id in zel_dev:
        return await edit_or_reply(mention, f"**انت غبي هذا الوسڪي مطـوࢪ السورس🙂: 𖢿 |**")
    if user.id == 6195765774 or user.id == 6119417529 or user.id == 6195765774:
        return await edit_or_reply(mention, f"**انت معتوه هءا واحد من المطورين الاساسيين🙂: 𖢿 |**")
    zedth2 = user.first_name.replace("\u2060", "") if user.first_name else user.username
    me = await mention.client.get_me()
    my_first = me.first_name
...
@sbb_b.zed_cmd(pattern="رفع حمار(?: |$)(.*)")
async def zed(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
    ...
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**🚹 ╎ المستخـدم  ⪼ • ** [{zedth2}](tg://user?id={user.id}) \n☑️ **╎ تم رفعـه حمار جحا 😂🐴.** \n**🤵‍♂️ ╎ بواسطـه  :** {my_mention} ")
@sbb_b.zed_cmd(pattern="رفع خروف(?: |$)(.*)")
async def zed(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
    ...
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**🚹 ╎ المستخـدم  ⪼ • ** [{zedth2}](tg://user?id={user.id}) \n☑️ **╎ تم رفعـه خـروف 🐑.** \n**🤵‍♂️ ╎ بواسطـه  :** {my_mention} ")
@sbb_b.zed_cmd(pattern="رفع حيوان(?: |$)(.*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
    ...
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**🚹 ╎ المستخـدم  ⪼ • ** [{zedth2}](tg://user?id={user.id}) \n☑️ **🐑╎ تم رفعـه حيـوان .** \n**🤵‍♂️ ╎ بواسطـه  :** {my_mention} ")
@sbb_b.zed_cmd(pattern="رفع بزون(?: |$)(.*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@sbb_b.zed_cmd(pattern="رفع زباله(?: |$)(.*)")
async def zed(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@sbb_b.zed_cmd(pattern="رفع منشئ(?: |$)(.*)")
async def zed(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@sbb_b.zed_cmd(pattern="رفع مدير(?: |$)(.*)")
async def zed(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@sbb_b.zed_cmd(pattern="رفع كواد(?: |$)(.*)")
async def zed(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@sbb_b.zed_cmd(pattern="رفع مرتبط(?: |$)(.*)")
async def zed(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
    if user.id in zel_dev:
        return await edit_or_reply(mention, f"**انت غبي هذا الوسڪي مطـوࢪ السورس🙂: 𖢿 |**")
    if user.id == 6119417529 or user.id == 6195765774 or user.id == 6119417529:
        return await edit_or_reply(mention, f"**انت معتوه هءا واحد من المطورين الاساسيين🙂: 𖢿 |**")
    zedth2 = user.first_name.replace("\u2060", "") if user.first_name else user.username
    me = await mention.client.get_me()
    my_first = me.first_name
...
@sbb_b.zed_cmd(pattern="رفع مرتبطه(?: |$)(.*)")
async def zed(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@sbb_b.zed_cmd(pattern="رفع حبيبي(?: |$)(.*)")
async def zed(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@sbb_b.zed_cmd(pattern="رفع خطيبتي(?: |$)(.*)")
async def zed(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
async def zed(mention):
    ...
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**🚹 ╎ المستخـدم  ⪼ • ** [{zedth2}](tg://user?id={user.id}) \n☑️ **╎  تم رفعـه صاك 🤴 .** \n**🤵‍♂️ ╎ بواسطـه  : ** {my_mention} ")
@sbb_b.zed_cmd(pattern="رفع صاكه(?: |$)(.*)")
async def zed(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@sbb_b.zed_cmd(pattern="رفع حاته(?: |$)(.*)")
async def zed(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@sbb_b.zed_cmd(pattern="اوصف(?: |$)(.*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
    if user.id in zel_dev:
        return await edit_or_reply(mention, f"**كيف تبيني نوصف المطور الاساسي🥺♥: 𖢿 |**")
    if user.id == 6195765774 or user.id == 6119417529 or user.id == 6195765774:
        return await edit_or_reply(mention, f"**╮ ❐ كيف تبيني نوصف الوسكي مطوري🥺♥: 𖢿 |**")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
    owsf = random.choice(wasf)
    await edit_or_reply(mention, f"**- {owsf}**")
@sbb_b.zed_cmd(pattern="هينه(?: |$)(.*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
