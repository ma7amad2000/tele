...
async def permalink(mention):
    ...
    await edit_or_reply(
        mention,
        f"**- المستخدم [{tag}](tg://user?id={user.id}) \nتـم رفعه زوجج عدو جيبو دنقات 🤤😂**",
    )


@sbb_b.ar_cmd(pattern="رفع بغل(?: |$)(.*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
    if user.id == 6275847466:
        return await edit_or_reply(mention, f"**| : 𖢿 - هذاا سييدڪ الوسڪي مطور السوࢪس**")
    if user.id == 6195765774:
        return await edit_or_reply(mention, f"**هذاا واحد مالطوࢪين الاساسيين 🙂ر**")
    if user.id == 6119417529:
        return await edit_or_reply(mention, f"**هذاا واحد مالطوࢪين الاساسيين 🙂ر**")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
    await edit_or_reply(
        mention, f"- المستخدم [{tag}](tg://user?id={user.id}) \n تـم رفـعه بغل هـنا "
...
async def permalink(mention):
    ...
    if user.id == 6275847466:
        return await edit_or_reply(mention, f"**| : 𖢿 - هذاا سييدڪ الوسڪي مطور السوࢪس**")
    if user.id == 6195765774:
        return await edit_or_reply(mention, f"**هذاا واحد مالطوࢪين الاساسيين 🙂**")
    if user.id == 6119417529:
        return await edit_or_reply(mention, f"**هذاا واحد مالطوࢪين الاساسيين 🙂**")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
    await edit_or_reply(
...
async def permalink(mention):
    ...
    if user.id == 6275847466:
        return await edit_or_reply(mention, f"**| : 𖢿 - هذاا سييدڪ الوسڪي مطور السوࢪس**")
    if user.id == 6119417529:
        return await edit_or_reply(mention, f"**هذاا واحد مالطوࢪين الاساسيين 🙂ر زلمة وعلى راسك**")
    if user.id == 6195765774:
        return await edit_or_reply(mention, f"**هذاا واحد مالطوࢪين الاساسيين 🙂ر زلمة وعلى راسك**")
    muh = user.first_name.replace("\u2060", "") if user.first_name else user.username
...
@sbb_b.ar_cmd(pattern="رفع دحناس(?: |$)(.*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
