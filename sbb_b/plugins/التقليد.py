...
async def echo(event):
    ...
    jmubevent = await edit_or_reply(event, "⌔∮ يتم تفعيل هذا الامر انتظر قليلا ،")
    user, rank = await get_user_from_event(event, jmubevent, nogroup=True)
    if not user:
        return
    ...
    try:
        ...
        await edit_delete(jmubevent, f"⌔∮ خطأ\n`{str(e)}`")
    else:
        await edit_or_reply(
            jmubevent,
