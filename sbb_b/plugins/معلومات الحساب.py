...
# =========================================================== #
STAT_INDICATION = "**
𖢿︙جـاري جـمـع الإحصـائيـات انتـظـر ⏱ **"
CHANNELS_STR = "**
𖢿︙قائمة القنوات التي أنت فيها موجودة هنا\n\n"
CHANNELS_ADMINSTR = "**
𖢿︙قائمة القنوات التي انت مشـرف بهـا **\n\n"
CHANNELS_OWNERSTR = "**
𖢿︙قائمة القنوات التي تـكون انت مالكـها**\n\n"
GROUPS_STR = "**
𖢿︙قائمة المجموعات التي أنت فيها موجود فيـها**\n\n"
GROUPS_ADMINSTR = "**
𖢿︙قائمة المجموعات التي تكون مشـرف بهـا**\n\n"
GROUPS_OWNERSTR = "**
𖢿︙قائمة المجموعات التي تـكون انت مالكـها**\n\n"
# =========================================================== #
#                                                             #
# =========================================================== #
...
async def stats(event):  # sourcery no-metrics
    ...
    response = f"✛━━━━━━━━━━━━━✛ \n"
    response += f"**
𖢿︙الدردشات الخاصة ️  :** {private_chats} \n"
    response += f"**
𖢿︙المستخـدمين : {private_chats - bots} \n"
    response += f"**
𖢿︙الـبوتـات :** {bots} \n"
    response += f"**
𖢿︙المجـموعـات :** {groups} \n"
    response += f"**
𖢿︙القنـوات  :** {broadcast_channels} \n"
    response += f"**
𖢿︙المجـموعات التـي تكـون فيها مشرف  :** {admin_in_groups} \n"
    response += f"**
𖢿︙المجموعات التـي تـكون انت مالكـها  **: {creator_in_groups} \n"
    response += f"**
𖢿︙القنوات التـي تكـون فيها مشـرف :** {admin_in_broadcast_channels} \n"
    response += (
        f"**
𖢿︙صلاحيات الاشـراف  :** {admin_in_broadcast_channels - creator_in_channels} \n"
    )
    response += f"**
𖢿︙المحـادثـات الغيـر مقـروء**: {unread} \n"
    response += f"**
𖢿︙الـتاكـات الغيـر مقـروء** : {unread_mentions} \n"
    response += f"✛━━━━━━━━━━━━━✛\n"
    ...
    if input_str:
        try:
            ...
            try:
                ...
                await edit_delete(
                    event, "
𖢿︙يجـب وضع ايدي الشخـص او معـرفه اولا"
                )
            uid = u.id
    else:
