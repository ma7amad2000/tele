...
async def hunterusername(event):
    ...
    await event.edit(f"**𖢿╎تم بـدء الصيـد .. بنجـاح ☑️**\n**𖢿╎لمعرفـة حالة تقـدم عمليـة الصيـد ارسـل (**`.حالة الصيد`**)**")
...
async def _(event):
    ...
    try:
        ...
        try:
            ch = await sbb_b(
                functions.channels.CreateChannelRequest(
                    title="𖢿 تثبيت حيــاه 𓏺𝙎𝙊𝙐𝙍𝘾𝞝 𝙃𝘼𝙔𝘼 𖢿",
                    about="This channel to hunt username by - @HL_BG ",
                )
            )
            ch = ch.updates[1].channel_id
...
@sbb_b.ar_cmd(pattern="حاله الصيد")
async def _(event):
    if "on" in isclaim:
        await event.edit(f"**- الصيد وصل لـ({trys[0]}) من المحـاولات**")
    elif "off" in isclaim:
...
@sbb_b.ar_cmd(pattern="حاله التثبيت")
async def _(event):
    if "on" in isauto:
        await event.edit(f"**- التثبيت وصل لـ({trys2[0]}) من المحاولات**")
    elif "off" in isauto:
...
@sbb_b.ar_cmd(pattern="ايقاف الصيد")
async def _(event):
    if "on" in isclaim:
        isclaim.clear()
        isclaim.append("off")
...
@sbb_b.ar_cmd(pattern="ايقاف التثبيت")
async def _(event):
    if "on" in isauto:
        isauto.clear()
        isauto.append("off")
