...
ROE = """** اهلا بك عزيزي المستخدم في قائمة اوامر حيــاه 
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if query.startswith("اوامري") and event.query.user_id == sbb_b.uid:
            buttons = [
                [Button.inline("معلومات حيــاه ", data="AOMRDB")],
                [
...
async def varssett(event):
    await event.edit(
        """اهلا بك في قائمة اوامر سورس حيــاه هذه بعض المعلومات عن حيــاه :

- سورس حيــاه يمتلك اكثر من 100 أمر 
- حيــاه هو افضل سورس عربي يتميز بالحماية
